...
public class ProxyCVACA{

	public static int TAM_MAX=815000;
				...
				if(url1!=null){	
					for(int j=0;j<(aL.size())-1;j++){
						...
						if(((url1.compareTo(aL.get(j)))>=0) && ((url1.compareTo(aL.get(j+1)))<0)){
							...
							//System.out.println(countLF+" seg");
							...
							for(int k=0;k<countLF;k++){
								if(accesoURL(k)!= null && !accesoURL(k).isEmpty()){
									...
									if(url1.equals(url2)){
										...
										memAct = usoMemoria(); 
										System.out.println("Uso de memoria: "+memAct+" bytes.");
										if(memAct>memMax){
											memMax=memAct;
