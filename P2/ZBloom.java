...
public class ZBloom{
	...
	public static void main (String args[]) throws IOException{
		...
		FileReader fxBL = new FileReader(fbl);		//Se cuentan las líneas del fichero a dividir
		LineNumberReader lnrBL = new LineNumberReader(fxBL);
		int countBL=0;
		while(lnrBL.readLine ()!= null){
			countBL ++;
		}
		lnrBL.close();
				...
				if(url1!=null&&bloomFilter.contains(url1)){ //Se comprueba en este punto si puede estar
					...
					while(inf1<=sup1 & encontrado1 == false) {  //Para acceder a ficheros utilizamos el algoritmo de búsqueda binaria
						...
						if(((url1.compareTo(aL.get(centro1)))>=0) && ((url1.compareTo(aL.get(centro1+1)))<0)) {
							...
							lnr2.close();
								//Podemos mantener la busqueda binaria en los subficheros, pero no en las urls
